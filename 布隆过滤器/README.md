...
![布隆过滤器查询流程.png](https://github.com/maixxx987/StudyNotes/blob/master/%E5%B8%83%E9%9A%86%E8%BF%87%E6%BB%A4%E5%99%A8/image/%E7%BC%93%E5%AD%98%E6%B5%81%E7%A8%8B.png)


### 2. 去重
在开发过程中，我们经常会遇到需要去重的场景。如处理数据中，已经处理过的数据不再处理；亦或者是像个性化推荐的场景，我们已经推荐过的内容不需要在重复推荐。

在数据量特别大，或我们不需要精确去重的时候，布隆过滤器就非常适合用来去重。

#### 流程
1. 先判断布隆过滤器是否存在，若不存在则创建对应的布隆过滤器(可以每个任务都创建一个)。
2. 判断需要处理的数据是否在布隆过滤器中
    1. 数据存在布隆过滤器中 -- 寻找下一条数据
    2. 数据不存在 -- 处理
3. 将数据添加进布隆过滤器。
4. 全部数据处理完成后，便可以将该任务的布隆过滤器删除

![布隆过滤器去重流程.png](https://github.com/maixxx987/StudyNotes/blob/master/%E5%B8%83%E9%9A%86%E8%BF%87%E6%BB%A4%E5%99%A8/image/%E5%8E%BB%E9%87%8D%E6%B5%81%E7%A8%8B.png)
